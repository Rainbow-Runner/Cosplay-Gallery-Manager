...
export function ManageCoreEntitiesPage({ coserOnly = false }: { coserOnly?: boolean }) {
  ...
  async function persistEntity() { if (!entityValid) return; setMessage(""); try { let saved: ManageCoreEntity; if (draft.uuid) { const result = await update({ variables: { uuid: draft.uuid, expectedMetadataRevision: draft.metadataRevision, input: input() } }); if (!result.data) throw new Error("Server did not return the saved entity"); saved = result.data.updateCoreEntity; } else { const result = await create({ variables: { input: input() } }); if (!result.data) throw new Error("Server did not return the created entity"); saved = result.data.createCoreEntity; updateListParameters({ uuid: saved.uuid }); } setDraft(saved); if (saved.kind === "CHARACTER") setPersistedCharacterWork({ uuid: saved.workUUID || "", name: saved.workName || saved.workUUID || "" }); await listQuery.refetch(); setMessage("Saved"); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save entity"); } }
  ...
  return <main className="manage-page"><header className="manage-heading"><div><p>PORTABLE UUID ENTITIES</p><h2>{coserOnly ? "Coser" : "Core entities"}</h2></div><button type="button" onClick={() => { updateListParameters({ uuid: null }); setDraft(blank(kind)); setAliasText(""); setAliasChips([]); setAliasChipDraft(""); }}>＋ New</button></header>{!coserOnly ? <nav className="task-filters">{kinds.map((value) => <button key={value} type="button" className={kind === value ? "is-active" : ""} onClick={() => setParameters({ kind: value })}>{value}</button>)}</nav> : null}{message ? <p className="manage-message">{message}</p> : null}
    ...
    </div>{listQuery.loading && !data ? <p className="entity-list-state">{intl.formatMessage({ id: "manage.entityList.loading" }, { kind: kindLabel })}</p> : listQuery.error ? <p className="entity-list-state manage-error">{intl.formatMessage({ id: "manage.entityList.failed" }, { kind: kindLabel })}</p> : data?.items.length === 0 ? <p className="entity-list-state">{intl.formatMessage({ id: "manage.entityList.empty" }, { kind: kindLabel })}</p> : null}{data?.items.map((item) => <button type="button" className={`${item.uuid === selected ? "is-active" : ""}${item.kind === "COSER" ? " is-coser" : ""}${item.kind === "CHARACTER" ? " is-character" : ""}`} key={item.uuid} onClick={() => updateListParameters({ uuid: item.uuid, page })}>{item.kind === "COSER" ? <ManageCoserListItem item={item} /> : item.kind === "CHARACTER" ? <><span className="entity-manage-list__copy"><strong>{item.name}</strong><span>{item.aliases.join(" / ") || item.uuid}</span></span><span className="entity-manage-list__work">{item.workName || item.workUUID || "—"}</span></> : <><strong>{item.name}</strong><span>{item.aliases.join(" / ") || item.uuid}</span></>}</button>)}{data ? <ManageEntityPagination data={data} kindLabel={kindLabel} onPage={(value) => updateListParameters({ page: value })} /> : null}</aside>
