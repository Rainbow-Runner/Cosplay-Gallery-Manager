...
- 1.5（媒体拍摄日期自动提取，开发完成待部署）：schema v15保存当前媒体修订的日期证据及手工日期冲突复核状态。来源扫描不在事务内逐个读取大文件；现有有界worker队列分批提取图片EXIF `DateTimeOriginal`与视频QuickTime/容器创建日期。只有全部可处理媒体完成后才用最早日期更新无冲突的Gallery `shoot_date`；手工值冲突须在Manage → Gallery → Basic确认。可移植重建恢复Manifest日期后在目标机重新排队提取，不迁移源机证据。正式业务库尚未执行v15迁移，须先备份并完成部署门禁。详见[1.5开发日志](V1_5_DEVELOPMENT_LOG.md)。
