...
- 1.5（Coser管理列表大数据量交互，本地完成）：Manage Coser不再只能逐页浏览固定30项；认证查询现支持在数据库分页前对主名称、Sort name和Alias执行全库包含搜索，并按缺头像、缺Banner、任一不完整或两者完整筛选。左侧列表提供300ms搜索、30/60/100页量、结果区间、首页/前页/页码输入/后页/末页和空结果/错误状态；工具区与分页区固定，人物行在中间滚动。`q/assets/pageSize/page/uuid`全部保留在URL，刷新、返回、深链及从重复身份提示打开已有Coser不会丢失列表上下文。没有新增表、索引或数据库schema。
