...
2026-09-10继续迁移产品化源码开发：新增与CLI共用服务的双语Web工作台，覆盖导出就绪报告、导出、空库导入、Merge审核/应用/终止、媒体库映射、Gallery重建预检/执行、可选连续性范围摘要/应用及中断恢复，所有迁移动作继续要求所有者重新认证和动作确认词。portable package升级到format v2并兼容读取v1；独立`owner-continuity.json`默认不存在，只按显式选择保存Gallery生命周期/首次收录时间或Gallery收藏隐藏/Item收藏，checksums和身份引用门禁完整覆盖。Gallery地址连续性取消；Gallery/Item评分继续由Manifest负责；最后浏览时间、最后浏览项目等轻量浏览历史不迁移，应用代码只更新所选字段并由测试确认目标评分/历史不变。连续性只能在`GALLERIES_REBUILT`后原子应用，ACTIVE请求重新执行当前环境门禁；无数据库schema变化。修复v2 Merge会话格式版本持久化，并把Web失败收敛为不泄露路径/正文的动作级稳定码。源码自动化门禁已闭合，本轮准备提交和部署；真实Linux→Docker迁移与恢复模拟由所有者在部署后执行。
