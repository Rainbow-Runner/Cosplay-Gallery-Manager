...
# 2026-09-19 媒体库扫描识别与默认排除规则帮助说明（已部署）
...
# 2026-09-19 Gallery Media目录折叠与紧凑列表（已部署）
...
- 覆盖目录树构建、空中间目录、子树移动边界、折叠/展开与Caption同行、缺失媒体入口的回归。TypeScript检查、Web 34文件130项测试、685模块生产构建通过，仅见既存主chunk大小提示。纯前端与开发文档变更，无后端或数据库schema改动。
...
- 数据库定向测试覆盖有封面和无封面结果，前端测试覆盖单行封面、占位和路由。正式三标签Browse/产品库/API/Server/CMD测试及Go Vet、TypeScript、前端35文件131项测试和685模块生产构建通过；仅见既存主chunk超过500KiB提示。
...
### 累计功能提交、完整回滚备份与无schema增量部署

- 用户要求将上述两项累计功能提交并部署。提交前正式三标签`mediaprocessing`、`processingworker`、`productdb`、`productapi`、`productserver`、`cmd/cgm`全部Go测试和同范围Go Vet重新通过，`git diff --check`通过；源码与开发记录提交为`9d0cd79aa7303ce2dcd0fa31b045f261cf180f8b`（`Prioritize manual Gallery processing and extract dates inline`）。从清洁提交构建的正式三标签二进制报告该完整修订，Go VCS元数据为`vcs.modified=false`，候选SHA-256为`b4513c851f84dbca4d059fd4f00a71830bc95afdf4dd457b69dbb93acab825bb`。无前端和schema变更，因此不执行前端重构建或数据库迁移。
...
### 提交、schema v16正式部署与Docker Hub发布

- 上述两项功能及阶段测试记录提交为`f424966a77495a768e2ad054daf70e116e85aebc`（`Simplify Docker setup and add owner recovery`）。从清洁提交构建正式`linux/amd64`三标签镜像，内嵌前端生产构建通过；镜像中二进制报告准确提交，SHA-256为`f90ba0fd9e10ec751ef35dca4121c50a5e9eecedc683b72021c3fc50a12bec66`。无业务数据隔离容器启动后Health为204、About指向该提交。
