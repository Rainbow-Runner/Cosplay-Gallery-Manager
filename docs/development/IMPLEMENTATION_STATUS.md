...
- 1.5（Coser管理列表图片完善度，本地完成）：Coser名称前新增40px圆形头像，已配置时复用认证的revision资源URL并延迟解码，未配置时保留等尺寸浅灰空白占位，资源加载失败与未设置区分为琼珀色错误状态。每行右侧增加低干扰头像/Banner图标，绿色表示已设置、灰色表示缺失，提供中英文悬浮说明和可访问名称。列表现有查询已包含`avatarURL/bannerURL`，因此不增加GraphQL请求、数据库或schema变更；Work/Character/Tag列表不受影响。
