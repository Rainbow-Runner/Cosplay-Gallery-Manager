...
// Touch records a successful generated-resource access for LRU ordering.
// The complete cache identity prevents an old immutable URL from refreshing a
// replacement derivative that happens to use the same item and variant.
func (s *DerivativeStore) Touch(ctx context.Context, itemUUID, variant string, contentRevision int64, profileHash string, now time.Time) error {
	result, err := s.db.ExecContext(ctx, `UPDATE media_derivatives SET last_accessed_at_utc=?
		WHERE item_uuid=? AND variant=? AND content_revision=? AND profile_hash=? AND state IN ('READY','STALE')`,
		formatTime(normalisedTime(now)), itemUUID, variant, contentRevision, profileHash)
	if err != nil {
		return err
	}
...
// CacheTierBytes reports database-referenced generated bytes. The filesystem
// total can be larger because it also includes temporary files and orphans.
func (s *DerivativeStore) CacheTierBytes(ctx context.Context) (base, enhanced int64, err error) {
	err = s.db.QueryRowContext(ctx, `SELECT
		COALESCE(SUM(CASE WHEN cache_tier='BASE' THEN byte_size ELSE 0 END),0),
		COALESCE(SUM(CASE WHEN cache_tier='ENHANCED' THEN byte_size ELSE 0 END),0)
		FROM media_derivatives`).Scan(&base, &enhanced)
	return
}

// AdoptOnDemandLightboxPolicy is the idempotent local data migration for the
// v1.5 retention policy. Existing generated Lightbox files remain available,
// but become reclaimable and future retries carry the ENHANCED tier invariant.
func (s *DerivativeStore) AdoptOnDemandLightboxPolicy(ctx context.Context) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	defer func() { _ = tx.Rollback() }()
	if _, err := tx.ExecContext(ctx, `UPDATE media_derivatives SET cache_tier='ENHANCED'
		WHERE variant=? AND cache_tier<>'ENHANCED'`, mediaprocessing.VariantLightbox4096); err != nil {
		return err
	}
	payload, err := json.Marshal(map[string]any{"cache_tier": mediaprocessing.CacheEnhanced})
	if err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE processing_jobs SET payload_json=?
		WHERE job_kind='ITEM_DERIVATIVE' AND variant=?`, payload, mediaprocessing.VariantLightbox4096); err != nil {
		return err
	}
	return tx.Commit()
}
