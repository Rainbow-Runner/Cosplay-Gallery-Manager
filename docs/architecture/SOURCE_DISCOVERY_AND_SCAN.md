...
来源内受支持媒体默认纳入。`excluded`属于人工业务决定，扫描只能更新文件事实，不能恢复它；1,000硬上限仅统计非排除Item。

显式忘记仅允许MISSING或已排除Item，只删除应用数据库中的成员记录，并把item_uuid写入永久Tombstone。该操作没有任何磁盘删除能力。

Position在Gallery内全局唯一。首次按PHOTO、SELFIE、动画、视频优先级和完整相对路径自然排序，以1,024间隔分配；后续新增只写全局高水位。拖拽仅在同媒体组内，优先计算单个新位置，间隔耗尽时只把当前组迁移到高水位后的新区间。
