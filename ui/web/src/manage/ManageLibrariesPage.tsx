  ...
  return <main className="manage-page">
    ...
    <section className="library-layout">
      <aside className="library-list"><h3>{f("manage.library.mediaLibraries")}</h3>{libraries.map((library) => <button type="button" className={library.id === selectedID ? "is-active" : ""} key={library.id} onClick={() => { setSelectedID(library.id); resetRuleEditor(); setDeletingRuleID(null); }}><strong>{library.name}</strong><span>{library.rootPath}</span></button>)}
        <details><summary>{f("manage.library.add")}</summary><form onSubmit={addLibrary}><label>{f("manage.library.nameRequired")}<input required value={libraryDraft.name} onChange={(event) => setLibraryDraft({ ...libraryDraft, name: event.target.value })} /></label><label>{f("manage.library.pathRequired")}<input required value={libraryDraft.rootPath} onChange={(event) => setLibraryDraft({ ...libraryDraft, rootPath: event.target.value })} placeholder="/media" /></label><p>{f("manage.library.dockerPathHelp")}</p><label>{f("manage.library.timezoneRequired")}<input required value={libraryDraft.captureTimezone} onChange={(event) => setLibraryDraft({ ...libraryDraft, captureTimezone: event.target.value })} /></label><label className="check"><input type="checkbox" checked={libraryDraft.metadataWritebackEnabled} onChange={(event) => setLibraryDraft({ ...libraryDraft, metadataWritebackEnabled: event.target.checked })} /> {f("manage.library.metadataWriteback")}</label><p>{f("manage.library.mediaReadOnlyHelp")}</p><button type="submit" disabled={!libraryValid || createLibraryState.loading}>{createLibraryState.loading ? f("manage.library.creating") : f("manage.library.create")}</button></form></details>
      </aside>
