...
export const MANAGE_RUNTIME_SETTINGS = gql`${RUNTIME_SETTINGS_FIELDS} query ManageRuntimeSettings { manageRuntimeSettings { ...RuntimeSettingsFields } }`;
...
const MANAGE_CORE_ENTITY_FIELDS = gql`fragment ManageCoreEntityFields on ManageCoreEntity { kind uuid name sortName aliases slug metadataRevision workUUID profileSummary biography countryOrRegion useInRecommendation socialAccounts { uuid platformKey label handle url status visible position } parents { uuid name metadataRevision } }`;
