...
const MANAGE_CORE_ENTITY_FIELDS = gql`fragment ManageCoreEntityFields on ManageCoreEntity { kind uuid name sortName aliases slug metadataRevision workUUID workName profileSummary biography countryOrRegion useInRecommendation allowDirectAssignment avatarURL bannerURL avatarCrop { x y size } bannerFocalPoint { x y } socialAccounts { uuid platformKey label handle url status visible position } parents { uuid name metadataRevision } }`;
...
export const MANAGE_TAG_TREE = gql`query ManageTagTree { manageTagTree { uuid name aliases parentUUIDs metadataRevision childCount galleryCount allowDirectAssignment } }`;
...
export const MANAGE_CORE_ENTITY_OPTIONS = gql`query ManageCoreEntityOptions($kind: SearchEntityKind!, $query: String!, $limit: Int!, $assignableOnly: Boolean = false) { manageCoreEntityOptions(kind: $kind, query: $query, limit: $limit, assignableOnly: $assignableOnly) { kind uuid name aliases workUUID workName metadataRevision } }`;
