...
- 本轮产品数据库自动化/迁移测试、productapi/productserver测试、React TypeScript检查及Vitest 29文件/82项均通过；schema v6尚未迁移正式业务数据库，也未增量部署。
...
1. 在schema v6部署后的测试窗口，对媒体库自动化执行真实媒体长时间单来源心跳、操作系统强制进程中断/服务重启接管和处理中取消延迟门禁；万级IN_SYNC Draft合成门禁已证明25项批量值无需因队列开销调整，但仍需根据真实扫描吞吐复核。
2. 为schema v6安排本机维护窗口：先创建并独立校验完整备份，再迁移正式业务数据库，以少量真实媒体库验证MANUAL默认、ASSISTED、TRUSTED阻断、后台进度、取消和重复运行；未获得部署授权前不执行。
