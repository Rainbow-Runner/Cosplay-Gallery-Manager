    ...
    {tab === "media" ? <section><div className="manage-inline-toolbar"><span>{draft.items.length} members · {draft.row.missingCount} missing · folder paths are relative to the Gallery root</span><label><input type="checkbox" checked={missingOnly} onChange={(event) => setMissingOnly(event.target.checked)} /> 仅显示缺失媒体 / Missing only</label>{draft.row.missingCount > 0 ? <button type="button" disabled={forgetMissingState.loading} onClick={forgetAllMissing}>{forgetMissingState.loading ? "Forgetting…" : "批量忘记缺失记录 / Forget all missing"}</button> : null}<button type="button" onClick={autoCover}>Reset cover to auto</button></div>
      ...
      <div className="manage-media-groups">{mediaGroups.map((mediaGroup) => <section className="manage-media-group" key={mediaGroup.key}><header><h3>{mediaGroup.label}</h3><span>{flattenGalleryMediaFolders(mediaGroup.folders).length}</span></header>
        {mediaGroup.folders.map((folder, folderIndex) => <section className="manage-media-folder" key={`${mediaGroup.key}:${folder.path || "root"}`}><header><div><strong>{folder.path || "Gallery root"}</strong><small>{folder.items.length} item{folder.items.length === 1 ? "" : "s"}</small></div><div>
          {folder.path ? <><button type="button" aria-label={`Move folder ${folder.path} up`} disabled={missingOnly || reorderState.loading || folderIndex === (mediaGroup.folders[0]?.path === "" ? 1 : 0)} onClick={() => moveFolder(mediaGroup.folders, folderIndex, -1)}>↑ Folder</button><button type="button" aria-label={`Move folder ${folder.path} down`} disabled={missingOnly || reorderState.loading || folderIndex === mediaGroup.folders.length - 1} onClick={() => moveFolder(mediaGroup.folders, folderIndex, 1)}>↓ Folder</button></> : <span className="manage-media-root-note">Always first</span>}
          <button type="button" disabled={missingOnly || reorderState.loading || folder.items.length < 2} onClick={() => naturalSortFolder(mediaGroup.folders, folderIndex)}>Natural sort filenames</button>
