...
- Gallery卡片固定3:4；2026-08-12经用户确认，索引网格改为与参考站一致的移动2列、`md` 3列、`lg` 4列、`2xl` 5列和16px统一间距。
...
- 主内容区：
  - 宽度为视口减去侧栏；
  - 桌面页面内容与64px品牌栏随文档向上滚动，左侧导航栏固定且不改变纵向位置；
  - 移动端因侧栏收起，64px品牌栏继续粘性固定以保留菜单入口；
...
- 此视口统一采用参考站4列、每列约268px、横向间距16px的索引网格；1536px起进入5列。
...
- 收藏和评分摘要；
- P/S/G/V计数；
- 响应式2/3/4/5列和16px间距（2026-08-12已替代原6/5/4/3/2列规则）。
...
- 保留3:4卡片；索引网格按后续确认采用参考站2/3/4/5列断点和16px统一间距，不影响Gallery详情媒体的独立2/3/4/6列规则。
...
移动端在侧栏改为Drawer后保留粘性品牌栏；桌面右侧Sticky辅助栏以视口顶部约16px为吸附位置，不再叠加已经滚离视口的品牌栏高度。
...
- 使用参考站的2/3/4/5列断点和16px统一间距，取消6列模式。
- 卡片改造为：
  1. 3:4封面；
  2. 封面右下角显示媒体数量，沿用`P / S / G / V`类型缩写，但数量为0的类型不得渲染；数量写在类型前，例如仅有100张Photo时显示`100P`，混合媒体可显示`96P 4S 2V`，四类全为0时隐藏整段；计数使用12px/16px、400字重白字，不使用背景、内边距、圆角、模糊或文字阴影；
  3. 封面右上角保留小型R-18三角；
  4. 第一文字行显示Character或Gallery标题；
  5. 第二行显示Work；Album没有Work时保留稳定空行或显示Gallery标题；
  6. 第三行显示Coser头像组和名称；
  7. 收藏语义不变；精确鼠标设备仅在封面悬浮或键盘聚焦时显示，触屏/粗指针设备始终显示；
...
- 顶部Breadcrumb；
- 中央Search；
- 采用参考站头像+名称轻量行；人物文字索引保留自身4/3/2列响应规则，不与Gallery卡片网格耦合；
- 移除四边框Tile矩阵；
- 使用参考站紧凑数字分页。
...
- 使用768/1024/1536断点形成2/3/4/5列，并统一16px间距。
...
- Coser人物文字索引4/3/2列和实体60项分页保持；人物详情内Gallery使用统一2/3/4/5列卡片网格。
...
- Coser详情Gallery隐藏重复的Coser行并继续使用统一3:4卡片；当时的6/5/4/3/2列约束已由2026-08-12确认的2/3/4/5列规则替代。
...
查询层只增加向后兼容的可选`collectionType`和实体索引`query`参数。类型仍由Cast实时推导，未新增数据库字段、Model表或Coser/Model角色字段；24/30/60分页保持不变，历史上的Gallery网格`6/5/4/3/2`列约束已由2026-08-12确认的2/3/4/5列规则替代。
...
### 2026-08-01：Gallery详情页第二轮高保真冻结参数

对用户指定的`https://galleryepic.com/zh/cosplay/10045`重新核对桌面与移动布局后，确认本轮按以下组合实施：

- 移除独立封面Hero，改为Breadcrumb、紧凑标题、媒体统计和右侧轻量操作区，媒体更早进入首屏；
- 主区媒体网格采用参考站断点：移动2列、`md` 3列、`lg` 4列、超宽屏6列，固定3:4比例和12px间距；此规则仅覆盖Gallery详情媒体，Gallery卡片索引后来单独调整为2/3/4/5列和16px间距；
- Photo/Selfie、GIF、Video仍按既定顺序和每组24项手工展开，筛选启用时仍按筛选结果导航；纯图片与混合媒体均隐藏分类标题，各组只依次展示，不改变后端分类或Item顺序；
- Related改为桌面256px窄栏和横向小缩略图条目，移动端顺延至媒体之后；不伪造浏览量、下载量、热门或下载按钮；
- Description、Credit/Cast、Work/Character、Tag、拍摄/收录时间、大小和ExternalLink不删除，收纳到紧凑的“更多详情”区域；
- 媒体Tile拆为主打开区域、收藏按钮与操作菜单，避免嵌套按钮；静态图片可设为封面，GIF/Video不提供该操作，有效封面在网格中显示弱化标记；
- Lightbox接入单媒体收藏、半星评分、静态图片设封面和媒体详情入口，并补齐`?item=<item_uuid>`深链接、浏览器返回关闭、筛选结果导航、关闭后自动展开定位与`last_item_id`记录；
- 单媒体收藏与静态图设封面不是新增产品设计：分别来自V1计划P07-06与P07-02。本轮只补齐Browse详情页前端入口，继续复用现有GraphQL mutation、乐观并发版本和Manifest `DB_DIRTY`语义。

### 2026-08-09：桌面品牌栏滚动与Gallery定位补充

- 实机对比确认GalleryEpic桌面Logo/品牌栏随文档向上滚动，固定的是左侧分区导航；CGM因此把桌面Topbar改为普通文档流，并把Related辅助栏吸附位置改为视口顶部16px。1024px以下没有常驻侧栏，继续保留粘性Topbar和Drawer入口。
- Gallery“更多详情”改为受控弹层，点击外部或按`Escape`关闭，内部点击不会误关闭；新增直达`/manage/gallery/:setID?tab=media`入口。
- 按用户确认，将已认证单所有者Gallery详情的绝对父目录摘要作为无路径Browse约束的有限例外：DIRECTORY返回非MISSING实际Item（包括excluded）的去重父目录，根目录优先、其余自然排序；ARCHIVE只显示归档所在目录。列表、卡片、成员DTO、资源URL仍不返回文件名、相对路径、指纹或缓存路径，也不增加文件管理器或外部命令操作。
- Playwright真实滚动回归验证桌面Topbar离开视口而Sidebar纵坐标不变，并验证移动Topbar计算样式仍为`sticky`；同一离线业务流程验证目录显示、Manage链接和外部点击关闭。

### 2026-08-11：Gallery详情标题换行修正

- 排查确认React只直接输出Gallery标题，没有插入`br`或拆分字符串；异常提前换行来自标题CSS的`max-width: 34ch`、右侧操作区占位和移动端固定30px字号组合。中文默认可在汉字间断行，因此该限制在中文标题上尤其明显。
- 桌面移除`34ch`上限并让标题使用Grid左列全部空间；仍保留标题/操作左右结构，只在实际剩余宽度不足时正常换行。
- 700px及以下改为单列Header，操作区位于标题与统计信息之后并右对齐；标题字号收敛为`clamp(1.5rem, 7vw, 1.875rem)`。不采用全局`nowrap + ellipsis`，避免详情页隐藏业务标题。
- Chromium几何回归使用中等长度桌面标题验证计算样式`max-width: none`及单行内容高度，并用390px、300字符无空格标题验证操作区不挤压标题、标题不产生横向溢出；移动截图基线同步更新。

### 2026-08-12：Gallery卡片尺寸、计数与收藏可见性对齐

- 重新核对`galleryepic.xyz/zh`公开DOM/CSS后，用户确认用参考站真实断点替代此前CGM的6/5/4/3/2列约束：小屏2列、768px起3列、1024px起4列、1536px起5列，横纵间距统一16px；3:4卡片比例、24项分页和Gallery详情媒体独立网格均不改变。
- 媒体数量保持右下8px定位及现有零值过滤、P/S/G/V顺序和空格分隔，只把视觉改为参考站12px/16px、400字重Inter白字，并移除黑底、内边距、圆角、模糊和额外字间距。
- 收藏仍是既有Gallery收藏Mutation，不新增产品语义。精确鼠标设备默认隐藏按钮，封面悬浮或任意卡片内键盘焦点时显示；触屏/粗指针设备始终显示，避免没有Hover时丢失操作入口。
- 离线Chromium以浏览器计算样式锁定计数样式、390/768/1024/1536四档列数、16px间距和收藏悬浮/键盘聚焦行为；Browse、Model详情桌面/移动视觉基线同步更新。

### 2026-08-13：人物头像数据链路与社交图标复核

- 实机数据与资源服务校核确认，已导入的Coser头像原图及`avatar-480`派生资源都存在，详情页也能以认证资源URL返回200；列表页缺图不是刮削或图片生成失败，而是两条独立的数据链路遗漏：实体索引GraphQL转换没有转发仓储已有的头像状态，Gallery卡片Credit摘要则根本没有头像资源身份字段。
- Browse新增专用、无路径的`PersonSummary`，只携带UUID、名称、头像是否可用及资源revision；API只据此生成`/resource/coser/{uuid}/{revision}/avatar-480`，不返回`avatar_path`、Coser根目录或任何物理路径。Coser/Model索引继续复用既有`EntityIndexItem.avatarURL`，Gallery卡片及详情Credit使用人物摘要，避免给Work、Character、Tag等通用实体无差别膨胀字段。
- Gallery卡片人物行根据参考站收敛为32px头像与行高、6px头像名称间距、14px/14px、500字重名称；样式只作用于人物链接，不连带放大右侧评分。Coser/Model索引既有36px头像、14px/500名称规格保持不变。
- Coser详情社交账号由文字首字母改为本地内嵌SVG：Twitter/X为20px，其余平台为24px，间距4px；ACTIVE使用深色、INACTIVE使用灰色，保留外链、可访问名称、键盘焦点和自定义Platform key的通用网站图标回退，不依赖参考站静态资源或运行时网络。
- GraphQL集成回归同时验证人物索引和Gallery卡片返回同一认证头像URL且不泄露物理路径；组件回归验证真实`img src`及SVG图标，离线Chromium用计算样式锁定Gallery人物行几何并继续通过0.2%截图门禁。
- 部署后实机复核发现，首轮社交图标只对齐了20/24px尺寸和4px间距，但Facebook、Instagram、微博、Patreon等仍是手工近似的混合描边/填充图形；同时CGM只渲染已有账号，而参考站固定渲染X、Facebook、Instagram、微博、Patreon、Linktree六项，并以灰色不可点击图标表示缺失账号。现已按2026-08-13实时DOM逐路径对齐六项实心SVG和固定顺序，颜色精确为深色`#101828`与灰色`#99a1af`。
- GalleryEpic导入的Linktree历史记录使用可扩展的`platform_key=website`及`label=Linktree`，前端现兼容Platform key、Label和`linktr.ee` Host三种识别方式，不需要修改数据库或破坏自定义平台设计。参考六项之外的可见账号继续按数据库顺序追加，未知平台仍使用通用网站图标。
