...
export function MediaDetailPage() {
  ...
  const detail = query.data.mediaDetail; const resource = detail.displayResource ?? detail.item.largeResource ?? detail.item.cardResource;
  ...
  return <main className="media-detail"><section className="media-detail__stage">{resourceURL && resource?.mimeType.startsWith("video/")
    ? <video src={resourceURL} controls playsInline /> : resourceURL ? <img src={resourceURL} alt={detail.item.caption} />
      : <span>{detail.item.processingState}</span>}{detail.item.caption ? <p>{detail.item.caption}</p> : null}</section>
