...
- 1.5（Manage Coser中英文/拼音排序，本地完成）：Coser管理列表不再使用SQLite二进制文本顺序；全库搜索和图片完善度筛选后，以项目已有`golang.org/x/text`固定zh-CN Unicode Collation，在同一不区分大小写的字母序中混排英文名称和中文拼音，再进行30/60/100项分页。人工Sort name保持最高优先级，可覆盖多音字或特殊读音；名称、原始文本和UUID依次稳定破同序。排序不依赖宿主机locale，不新增依赖、字段、表或数据库schema。
