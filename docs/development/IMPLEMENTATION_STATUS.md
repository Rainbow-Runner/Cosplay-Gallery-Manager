...
- 1.5（后台帮助页说明媒体库识别与排除规则，本地源码完成、未部署）：Manage → Help 新增中英双语主题，解释媒体库发现、已有／忽略来源、有效Manifest、内置存档根、需要启用规则的`.cosplay-root`、PATH_TEMPLATE、FIXED_DEPTH及未分配媒体的处理顺序；区分根层级新媒体默认排除、自定义EXCLUDE/INCLUDE规则与安全／格式原因跳过。明确当前没有预装文件夹名或文件名排除清单，自定义INCLUDE也不能覆盖独立的根层级默认排除。仅前端和文档变更，TypeScript、34文件127项测试及685模块生产构建通过。详见[1.5开发日志](V1_5_DEVELOPMENT_LOG.md)。
