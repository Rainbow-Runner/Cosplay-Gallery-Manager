...
import { ADD_COSER_SOCIAL_ACCOUNT, CREATE_CORE_ENTITY, MANAGE_CORE_ENTITIES, MANAGE_CORE_ENTITY, MANAGE_CORE_ENTITY_NAME_CONFLICTS, MANAGE_COSER_MANIFEST, MANAGE_COSER_NAME_CONFLICTS, MANAGE_TAG_TREE, MANAGE_WORK_CHARACTERS, PULL_COSER_MANIFEST, PUSH_COSER_MANIFEST, REPLACE_TAG_PARENTS, RESOLVE_COSER_MANIFEST, UPDATE_CORE_ENTITY } from "../api/manage";
...
import type { ManageCoreEntity, ManageCoreEntityNameConflict, ManageCoreEntityPage, ManageCoserManifestState, ManageCoserNameConflict, ManageTagTreeItem } from "./types";
...
const blank = (kind: Kind): ManageCoreEntity => ({ kind, uuid: "", name: "", sortName: "", aliases: [], slug: "", metadataRevision: 0, workName: "", profileSummary: "", biography: "", countryOrRegion: "", useInRecommendation: true, allowDirectAssignment: true, socialAccounts: [], parents: [] });
...
export function ManageCoreEntitiesPage({ coserOnly = false }: { coserOnly?: boolean }) {
  ...
  const [tagView, setTagView] = useState<"TREE" | "LIST">("TREE");
  const [newTagParent, setNewTagParent] = useState<ManageTagTreeItem | null>(null);
  ...
  const [moveConfirmation, setMoveConfirmation] = useState(false); const [moveConfirmationText, setMoveConfirmationText] = useState("");
  const [persistedCharacterWork, setPersistedCharacterWork] = useState({ uuid: "", name: "" });
  ...
  const tagTreeQuery = useQuery<{ manageTagTree: ManageTagTreeItem[] }>(MANAGE_TAG_TREE, { skip: kind !== "TAG", fetchPolicy: "network-only" });
  ...
  useEffect(() => { const entity = detailQuery.data?.manageCoreEntity; setDraft(entity ?? blank(kind)); setPersistedCharacterWork(entity?.kind === "CHARACTER" ? { uuid: entity.workUUID || "", name: entity.workName || entity.workUUID || "" } : { uuid: "", name: "" }); setMoveConfirmation(false); setMoveConfirmationText(""); }, [detailQuery.data, kind, selected]);
  ...
  useEffect(() => { if (kind !== "TAG") setNewTagParent(null); }, [kind]);
  useEffect(() => {
  ...
  function input(value = draft) { return { kind: value.kind, name: value.name, sortName: value.sortName, aliases: value.kind === "WORK" || value.kind === "CHARACTER" ? aliasChips : parseAliases(aliasText), workUUID: value.kind === "CHARACTER" ? value.workUUID || null : null, profileSummary: value.profileSummary, biography: value.biography, countryOrRegion: value.countryOrRegion, useInRecommendation: value.useInRecommendation, ...(value.kind === "TAG" ? { allowDirectAssignment: value.allowDirectAssignment ?? true } : {}) }; }
  ...
  const directTagGalleryCount = draft.kind === "TAG" ? tagTreeQuery.data?.manageTagTree.find((tag) => tag.uuid === draft.uuid)?.galleryCount ?? 0 : 0;
  ...
  async function persistEntity() { if (!entityValid) return; setMessage(""); try { let saved: ManageCoreEntity; if (draft.uuid) { const result = await update({ variables: { uuid: draft.uuid, expectedMetadataRevision: draft.metadataRevision, input: input() } }); if (!result.data) throw new Error("Server did not return the saved entity"); saved = result.data.updateCoreEntity; } else { const result = await create({ variables: { input: { ...input(), tagParentUUID: draft.kind === "TAG" ? newTagParent?.uuid || null : null, expectedTagParentRevision: draft.kind === "TAG" ? newTagParent?.metadataRevision || null : null } } }); if (!result.data) throw new Error("Server did not return the created entity"); saved = result.data.createCoreEntity; updateListParameters({ uuid: saved.uuid }); setNewTagParent(null); } setDraft(saved); if (saved.kind === "CHARACTER") setPersistedCharacterWork({ uuid: saved.workUUID || "", name: saved.workName || saved.workUUID || "" }); await listQuery.refetch(); if (kind === "TAG") await tagTreeQuery.refetch(); setMessage("Saved"); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save entity"); } }
  async function save(event: FormEvent) { event.preventDefault(); if (!entityValid) return; if (draft.uuid && draft.kind === "CHARACTER" && persistedCharacterWork.uuid && draft.workUUID !== persistedCharacterWork.uuid) { setMoveConfirmationText(""); setMoveConfirmation(true); return; } await persistEntity(); }
  async function confirmCharacterMove() { if (moveConfirmationText !== "MOVE") return; setMoveConfirmation(false); await persistEntity(); }
  ...
  async function saveTagParents() { if (draft.kind !== "TAG" || !draft.uuid) return; const expected = new Map<string, number>(); for (const parent of detailQuery.data?.manageCoreEntity.parents || []) expected.set(parent.uuid, parent.metadataRevision); for (const parent of draft.parents) expected.set(parent.uuid, parent.metadataRevision); setMessage(""); try { const result = await replaceTagParents({ variables: { childUUID: draft.uuid, expectedChildRevision: draft.metadataRevision, parents: draft.parents.map((parent, index) => ({ uuid: parent.uuid, position: String((index + 1) * 1024) })), expectedParents: Array.from(expected, ([uuid, metadataRevision]) => ({ uuid, metadataRevision })) } }); if (result.data) setDraft(result.data.replaceTagParents); await detailQuery.refetch(); await listQuery.refetch(); await tagTreeQuery.refetch(); setMessage("Tag parent DAG saved"); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save Tag parents"); } }
  async function merged(target: ManageCoreEntity, completionWarning?: string | null) { setDraft(target); updateListParameters({ uuid: target.uuid }); await listQuery.refetch(); if (kind === "TAG") await tagTreeQuery.refetch(); setMessage(completionWarning ? `Merged. ${completionWarning}` : "Merged into the selected target; the source UUID is now a permanent alias."); }
  async function deleted() { setDraft(blank(kind)); updateListParameters({ uuid: null }); await listQuery.refetch(); if (kind === "TAG") await tagTreeQuery.refetch(); setMessage("Entity deleted from the database and UUID permanently tombstoned; files were not removed."); }
  return <main className="manage-page"><header className="manage-heading"><div><p>PORTABLE UUID ENTITIES</p><h2>{coserOnly ? "Coser" : "Core entities"}</h2></div><button type="button" onClick={() => { updateListParameters({ uuid: null }); setDraft(blank(kind)); setAliasText(""); setAliasChips([]); setAliasChipDraft(""); setNewTagParent(null); }}>＋ New</button></header>{!coserOnly ? <nav className="task-filters">{kinds.map((value) => <button key={value} type="button" className={kind === value ? "is-active" : ""} onClick={() => setParameters({ kind: value })}>{value}</button>)}</nav> : null}{message ? <p className="manage-message">{message}</p> : null}
    <section className="entity-manage-layout"><aside className="entity-manage-list"><div className="entity-list-tools">
      ...
      {kind === "TAG" ? <div className="manage-tag-view-switch"><button type="button" className={tagView === "TREE" ? "is-active" : ""} onClick={() => setTagView("TREE")}>{intl.formatMessage({ id: "manage.tagTree.tree" })}</button><button type="button" className={tagView === "LIST" ? "is-active" : ""} onClick={() => setTagView("LIST")}>{intl.formatMessage({ id: "manage.tagTree.list" })}</button></div> : null}
      {kind !== "TAG" || tagView === "LIST" ? <p className="entity-list-tools__sort-note">{intl.formatMessage({ id: "manage.entityList.sortNote" })}</p> : null}
      ...
      {kind !== "TAG" || tagView === "LIST" ? <label>{intl.formatMessage({ id: "manage.entityList.pageSize" })}<select value={pageSize} onChange={(event) => updateListParameters({ pageSize: Number(event.target.value) === defaultPageSize ? null : event.target.value, page: 1 })}>{entityPageSizes.map((value) => <option value={value} key={value}>{value}</option>)}</select></label> : null}
    ...
    </div>{kind === "TAG" && tagView === "TREE" ? <>
      {tagTreeQuery.loading && !tagTreeQuery.data ? <p className="entity-list-state">{intl.formatMessage({ id: "manage.tagTree.loading" })}</p> : tagTreeQuery.error ? <p className="entity-list-state manage-error">{intl.formatMessage({ id: "manage.tagTree.failed" })}</p> : tagTreeQuery.data ? <ManageTagHierarchy items={tagTreeQuery.data.manageTagTree} selected={selected} query={listSearch} onSelect={(uuid) => { setNewTagParent(null); updateListParameters({ uuid }); }} onCreateChild={(parent) => { setNewTagParent(parent); updateListParameters({ uuid: null }); setDraft(blank("TAG")); setAliasText(""); }} /> : null}
    </> : <>{listQuery.loading && !data ? <p className="entity-list-state">{intl.formatMessage({ id: "manage.entityList.loading" }, { kind: kindLabel })}</p> : listQuery.error ? <p className="entity-list-state manage-error">{intl.formatMessage({ id: "manage.entityList.failed" }, { kind: kindLabel })}</p> : data?.items.length === 0 ? <p className="entity-list-state">{intl.formatMessage({ id: "manage.entityList.empty" }, { kind: kindLabel })}</p> : null}{data?.items.map((item) => <button type="button" className={`${item.uuid === selected ? "is-active" : ""}${item.kind === "COSER" ? " is-coser" : ""}${item.kind === "CHARACTER" ? " is-character" : ""}`} key={item.uuid} onClick={() => { setNewTagParent(null); updateListParameters({ uuid: item.uuid, page }); }}>{item.kind === "COSER" ? <ManageCoserListItem item={item} /> : item.kind === "CHARACTER" ? <><span className="entity-manage-list__copy"><strong>{item.name}</strong><span>{item.aliases.join(" / ") || item.uuid}</span></span><span className="entity-manage-list__work">{item.workName || item.workUUID || "—"}</span></> : <><strong>{item.name}</strong><span>{item.aliases.join(" / ") || item.uuid}</span></>}</button>)}{data ? <ManageEntityPagination data={data} kindLabel={kindLabel} onPage={(value) => updateListParameters({ page: value })} /> : null}</>}</aside>
      <div className="entity-editor">{kind === "TAG" && newTagParent && !draft.uuid ? <p className="manage-tag-child-context">{intl.formatMessage({ id: "manage.tagTree.newChildContext" }, { name: newTagParent.name })}<button type="button" onClick={() => setNewTagParent(null)}>{intl.formatMessage({ id: "manage.tagTree.clearParent" })}</button></p> : null}{kind === "COSER" && draft.uuid ? <nav className="editor-tabs">{["profile", "social", "galleries", "manifest"].map((tab) => <button key={tab} type="button" className={coserTab === tab ? "is-active" : ""} onClick={() => setCoserTab(tab)}>{tab}</button>)}</nav> : null}
        ...
        {kind === "TAG" ? <section className="manage-panel tag-assignment-setting"><h3>{intl.formatMessage({ id: "manage.tagAssignment.heading" })}</h3><label className="settings-check"><input type="checkbox" checked={draft.allowDirectAssignment !== false} disabled={Boolean(draft.uuid) && (!tagTreeQuery.data || directTagGalleryCount > 0) && draft.allowDirectAssignment !== false} onChange={(event) => setDraft({ ...draft, allowDirectAssignment: event.target.checked })} />{intl.formatMessage({ id: "manage.tagAssignment.allow" })}</label><p>{intl.formatMessage({ id: "manage.tagAssignment.help" })}</p>{directTagGalleryCount > 0 ? <p className="manage-error">{intl.formatMessage({ id: "manage.tagAssignment.directCount" }, { count: directTagGalleryCount })}</p> : null}<p>{intl.formatMessage({ id: "manage.tagAssignment.saveHint" })}</p></section> : null}
    ...
    {moveConfirmation && draft.kind === "CHARACTER" ? <Dialog titleID="character-work-move-title" dismissible={!updateState.loading} onClose={() => setMoveConfirmation(false)}>
      <p>{intl.formatMessage({ id: "manage.characterMove.eyebrow" })}</p><h3 id="character-work-move-title">{intl.formatMessage({ id: "manage.characterMove.title" }, { character: draft.name })}</h3>
      <ul><li>{intl.formatMessage({ id: "manage.characterMove.route" }, { source: persistedCharacterWork.name || persistedCharacterWork.uuid || "—", target: draft.workName || draft.workUUID || "—" })}</li><li>{intl.formatMessage({ id: "manage.characterMove.identity" })}</li><li>{intl.formatMessage({ id: "manage.characterMove.references" })}</li><li>{intl.formatMessage({ id: "manage.characterMove.conflict" })}</li></ul>
      <label>{intl.formatMessage({ id: "manage.characterMove.typeToContinue" })}<input autoFocus value={moveConfirmationText} onChange={(event) => setMoveConfirmationText(event.target.value)} /></label>
      <footer><button type="button" onClick={() => setMoveConfirmation(false)}>{intl.formatMessage({ id: "manage.lifecycle.cancel" })}</button><button className="danger" type="button" disabled={moveConfirmationText !== "MOVE" || updateState.loading} onClick={confirmCharacterMove}>{updateState.loading ? intl.formatMessage({ id: "manage.lifecycle.committing" }) : intl.formatMessage({ id: "manage.characterMove.commit" })}</button></footer>
    </Dialog> : null}</main>;
}
