...
- 定向数据库测试覆盖Work Alias、Character主名/Alias、跨Work上下文、大小写/空格规范化、非精确值和非法Kind；API测试覆盖关联Work与主名命中标识。前端完整30文件97项、TypeScript、681模块生产构建，以及`productdb/productapi/productserver/ui/web/cmd/cgm`正式三标签组合测试通过；仅保留既有主共享包超过500KiB提示。本轮无数据迁移，源码尚未提交或部署。
