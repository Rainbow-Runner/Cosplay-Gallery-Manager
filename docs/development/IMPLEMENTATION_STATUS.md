...
- 1.5（Tag 分类容器与直接关联策略，源码完成、未部署）：Tag 新增“允许直接关联到图集”独立开关；关闭后仍可作为多父层级中的分类节点并汇总子标签图集，但 Browse/Manage 的 Gallery Tag 选择器不再提供它，数据库事务及触发器也禁止直接关联。已有直接关联的 Tag 必须先移除这些关系才能关闭开关；旧 Tag 在 schema v11→v12 迁移后默认保持可直接关联。可移植数据包记录此策略，旧包缺省解释为允许，导入和合并沿用同一字段；包格式版本不变。当前正式服务和业务库仍运行旧版本/schema v11，部署前须先做一致性备份，迁移后再验证导入/合并与实际浏览。本段的具体测试和边界见[1.5开发日志](V1_5_DEVELOPMENT_LOG.md)。
