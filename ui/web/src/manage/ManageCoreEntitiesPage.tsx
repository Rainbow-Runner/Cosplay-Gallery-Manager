...
import { ADD_COSER_SOCIAL_ACCOUNT, CREATE_CORE_ENTITY, MANAGE_CORE_ENTITIES, MANAGE_CORE_ENTITY, MANAGE_COSER_MANIFEST, MANAGE_COSER_NAME_CONFLICTS, PULL_COSER_MANIFEST, PUSH_COSER_MANIFEST, REPLACE_TAG_PARENTS, RESOLVE_COSER_MANIFEST, UPDATE_CORE_ENTITY } from "../api/manage";
...
import type { ManageCoreEntity, ManageCoreEntityPage, ManageCoserManifestState, ManageCoserNameConflict } from "./types";
...
function AliasChipInput({ value, onChange, draft, setDraft }: { value: string; onChange: (value: string) => void; draft: string; setDraft: (value: string) => void }) {
  const intl = useIntl();
  const inputID = useId();
  ...
  function replaceAliases(next: string[]) { onChange(next.join(" / ")); setError(""); }
...
export function ManageCoreEntitiesPage({ coserOnly = false }: { coserOnly?: boolean }) {
  ...
  const [draft, setDraft] = useState<ManageCoreEntity>(blank(kind)); const [aliasText, setAliasText] = useState(""); const [aliasChipDraft, setAliasChipDraft] = useState(""); const [message, setMessage] = useState(""); const [coserTab, setCoserTab] = useState("profile");
  ...
  const newCoserName = draft.kind === "COSER" && !draft.uuid ? draft.name.trim() : "";
  const duplicateQuery = useQuery<{ manageCoserNameConflicts: ManageCoserNameConflict[] }>(MANAGE_COSER_NAME_CONFLICTS, { variables: { name: checkedCoserName || "_", limit: 10 }, skip: checkedCoserName === "", fetchPolicy: "network-only" });
  ...
  useEffect(() => { setAliasText(draft.aliases.join(" / ")); setAliasChipDraft(""); }, [draft.kind, draft.metadataRevision, draft.uuid]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setCheckedCoserName(newCoserName), 350);
    return () => window.clearTimeout(timeout);
  }, [newCoserName]);
  ...
  const conflictsCurrent = checkedCoserName === newCoserName && !duplicateQuery.loading;
  const duplicateConflicts = conflictsCurrent ? duplicateQuery.data?.manageCoserNameConflicts || [] : [];
  const duplicateReady = !newCoserName || (conflictsCurrent && !duplicateQuery.error && (duplicateConflicts.length === 0 || duplicateConfirmed)) || (conflictsCurrent && Boolean(duplicateQuery.error) && duplicateConfirmed);
  ...
  return <main className="manage-page"><header className="manage-heading"><div><p>PORTABLE UUID ENTITIES</p><h2>{coserOnly ? "Coser" : "Core entities"}</h2></div><button type="button" onClick={() => { updateListParameters({ uuid: null }); setDraft(blank(kind)); setAliasText(""); setAliasChipDraft(""); }}>＋ New</button></header>{!coserOnly ? <nav className="task-filters">{kinds.map((value) => <button key={value} type="button" className={kind === value ? "is-active" : ""} onClick={() => setParameters({ kind: value })}>{value}</button>)}</nav> : null}{message ? <p className="manage-message">{message}</p> : null}
