...
- 1.5（Work/Character/Tag管理列表交互，源码完成、待部署）：三类核心实体前端不再隐藏已有的全库搜索与30/60/100页量能力，现提供双语搜索、排序规则说明、清除条件、结果区间、首页/前后页/页码/末页和URL状态恢复；搜索覆盖名称、Sort name与Alias。后端把Coser已确认的英文/中文拼音及Sort name优先排序推广到全部核心实体，并在全局排序后分页。Coser图片完善度筛选保持专属，无数据库或schema变化。
