...
- 1.5（Manifest定期只读巡检与手动批量Push，源码完成、待部署）：产品库schema v13将巡检观察与同步基线分离；已启用自动扫描时，每轮在来源扫描后按持久游标、有界批量检查已启用媒体库的Gallery Manifest，仅更新数据库巡检结果，不写来源文件。Gallery管理列表增加Manifest状态、检查时间及待处理筛选。最多选择100个Gallery先预览实际业务内容变化和本地文件变化，再明确选择跳过或覆盖本地改动；`DB_DIRTY`且业务内容确实变化时正常Push，只有revision变化而内容不变则不重写。执行逐项复核revision、路径、文件哈希；错误、来源不可用及只读媒体库始终跳过，逐项报告结果。提交与正式schema v13迁移、部署结果将在下条日志记录。详见[1.5开发日志](V1_5_DEVELOPMENT_LOG.md)。
